...
## MM bot — order book depth

- Deposits collateral (typically equal to vault TVL) and posts a two-sided limit-order ladder around fair value.
- Quote safety keeps the ladder well-formed:
  - **Bids** capped at oracle rate
  - **Asks** floored at `max(oracle, vAMM)`
  - Both sides clamped inside the live book
- Runs **vAMM↔oracle arbitrage** when implied rate deviates > 100 bps, re-anchoring the pool and limiting vault counterparty swings.
...
3. **Arb loop anchors vAMM.** After large vAMM swaps, the MM corrects rate divergence toward oracle.

At baseline launch throughput (~43 hedger arrivals/month), the split is roughly **55% LOB / 45% vAMM**. LOB share rises as depth grows; at very high throughput the vAMM share climbs again (throughput ceiling).
